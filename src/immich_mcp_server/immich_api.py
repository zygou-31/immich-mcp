...
class ImmichAPI:
    ...
    def __init__(self, api_url: str | None = None, api_key: str | None = None):
        ...
        if not self.api_url:
            raise ValueError("Immich API URL must be provided via argument or IMMICH_API_URL environment variable.")
        if not self.api_key:
            raise ValueError("Immich API key must be provided via argument or IMMICH_API_KEY environment variable.")
    ...
    async def ping_server(self) -> bool:
        ...
        try:
            response = await self._client.get("/server-info/ping")
